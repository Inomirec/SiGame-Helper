...
async def status(refresh: bool = False) -> dict:
    ...
    return {
        ...
        "app": "sigame-helper",
        # Сколько задач очереди ведут прямо сейчас. Может отличаться от
        # настроек, если их поменяли и ещё не применили.
        "pools": jobs.manager.sizes(),
        "version": __version__,
        "hardware": hardware,
...
@router.post("/pools")
async def apply_pools() -> dict:
    """Приводит очереди в соответствие с настройками без перезапуска.

    Число одновременных задач задаётся при старте, поэтому раньше изменение
    настройки ждало перезапуска программы. Здесь очередь просто получает или
    отпускает исполнителей: начатые файлы дорабатываются до конца.
    """
    return {"pools": jobs.apply_limits()}
