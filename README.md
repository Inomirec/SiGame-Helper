...
Ставить ничего не нужно — ни Python, ни ffmpeg, ни что-либо ещё.

1. Скачайте программу: **Code → Download ZIP** или архив из
   [Releases](../../releases).
2. Распакуйте папку куда удобно.
3. Запустите **`SiGame Helper.exe`**.

Первый запуск покажет окно подготовки с полосой прогресса и спросит, класть
ли ярлык на рабочий стол. Дальше `.exe` открывает программу сразу, без
всяких окон.
...
> Нажмите **«Подробнее» → «Выполнить в любом случае»**. Чаще всего этого
> окна не будет вовсе: оно появляется, только если архив распакован
> встроенным распаковщиком Windows, а не 7-Zip или WinRAR.

> Если антивирус удалит `.exe` — рядом лежит **`Запасной запуск.bat`**,
> он делает ровно то же самое.

При первом запуске программа сама скачает всё, что ей нужно: переносимый
Python (11 МБ), библиотеки, загрузчики `yt-dlp` и `gallery-dl`, **ffmpeg**
(около 160 МБ) и **Deno** (около 40 МБ). Это занимает несколько минут
**один раз** — дальше запуск мгновенный.

Всё складывается в папки `runtime` и `bin` рядом с программой и **не трогает
систему**. Удалить программу = удалить папку, ничего не остаётся.
...
Если первый запуск оборвался на полпути (пропал интернет, вмешался антивирус) —
просто запустите `SiGame Helper.bat` ещё раз, он продолжит с того же места.

> Если хотите, чтобы программа пользовалась **вашим** ffmpeg — положите
> `ffmpeg.exe` и `ffprobe.exe` в папку `bin` или оставьте их в PATH. Только
> берите **полную** сборку (например, <https://www.gyan.dev/ffmpeg/builds/>):
> в ней есть кодеры `libsvtav1`, `libaom-av1` (для AVIF) и `libopus`,
> в урезанных их нет.
