...
Единственное, что нужно поставить руками — **Python 3.11 или новее**:
<https://www.python.org/downloads/>. При установке обязательно поставьте
галочку **«Add python.exe to PATH»**.

Дальше просто запустите **`SiGame Helper.bat`** — и всё. При первом запуске он
сам создаст изолированное окружение, поставит `yt-dlp` и `gallery-dl`, скачает
**ffmpeg** (около 160 МБ, вместе с ffprobe) и **Deno** (около 40 МБ) в папку
`bin` и только потом откроет приложение. Это занимает пару минут один раз;
дальше запуск мгновенный.
...
Никаких кнопок в настройках нажимать не нужно — если чего-то не хватает,
программа доустановит это сама.

> Если предпочитаете поставить ffmpeg сами — берите **полную** сборку
> (например, <https://www.gyan.dev/ffmpeg/builds/>): в ней есть кодеры
> `libsvtav1`, `libaom-av1` (для AVIF) и `libopus`. В урезанных сборках их нет.
> Программа найдёт ffmpeg в PATH или в папке `bin` рядом с собой.

Отдельный файл `Установка.bat` тоже остался — он делает то же самое, но без
запуска приложения.
...
| Файл | Что делает |
| --- | --- |
| `SiGame Helper.bat` | Отдельное окно приложения (PyWebView / WebView2) |
| `SiGame Helper (браузер).bat` | Поднимает сервер и открывает вкладку в браузере |
| `Обновить загрузчики.bat` | Обновляет `yt-dlp` и `gallery-dl` |

Сайты постоянно ломают парсеры, поэтому `yt-dlp` стоит обновлять примерно раз
в месяц — либо этим файлом, либо кнопкой в настройках приложения.

С `ffmpeg` так делать не нужно: он не зависит от вёрстки сайтов и не «протухает».
Кнопка «Обновить» рядом с ним нужна только на случай, если понадобился более
свежий кодек.
...
6. Результат — в подпапке `_processed` рядом с исходником. Оригинал не трогается.
