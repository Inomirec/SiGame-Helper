...
3. Запустите **`SiGame Helper.bat`**.

При первом запуске откроется чёрное окно и программа сама скачает всё, что ей
нужно: переносимый Python (11 МБ), библиотеки, загрузчики `yt-dlp` и
`gallery-dl`, **ffmpeg** (около 160 МБ) и **Deno** (около 40 МБ). Это занимает
несколько минут **один раз** — дальше запуск мгновенный.
